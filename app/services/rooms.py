...
from app.schemas.rooms import RoomAddRequest, Room, RoomAdd, RoomPatchRequest, RoomPatch
...
class RoomService(BaseService):
    ...
    async def change_room(self, hotel_id: int, room_id: int, room_data: RoomAddRequest):
        await HotelService(self.db).get_hotel_with_check(hotel_id)
    ...
    async def changes_room(self, hotel_id: int, room_id: int, room_data: RoomPatchRequest):
        await HotelService(self.db).get_hotel_with_check(hotel_id)
        await self.get_room_with_check(room_id)
        _room_data_dict = room_data.model_dump(exclude_unset=True)
        _room_data = RoomPatch(hotel_id=hotel_id, **_room_data_dict)
        await self.db.rooms.edit(_room_data, hotel_id=hotel_id, id=room_id, exclude_unset=True)
